...
namespace Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public IActionResult Index(int? page, string searchString, string IdCate = "0")
        {
            ...
            if (IdCate != "0")
            {
                lsProduct = _context.Products
                                    .AsNoTracking()
                                    .Where(x => x.IdCate.Equals(IdCate) && x.StatusPro == 1).Include(e => e.IdAccNavigation)
            ...
            else
            {
                ...
                lsProduct = _context.Products
                                    .AsNoTracking()
                                    .Where(x => x.StatusPro == 1).Include(e=>e.IdAccNavigation)
                                    .Include(p => p.ProductItems).Include(i => i.ImgPros).Include(r => r.Reviews)
                                    .OrderByDescending(p => p.UpdateDate != null ? p.UpdateDate : p.CreateDate).ToList();
            }
        ...
        public IActionResult Detail(string id)
        {
            try
            {
                ...
                var query2 = from p in _context.Products.Include(i => i.ImgPros).Include(p => p.ProductItems).Include(p => p.IdCateNavigation).Include(r => r.Reviews)
                             join c in _context.Categories on p.IdCate equals c.IdCate
                             join pI in _context.ProductItems on p.IdPro equals pI.IdPro
                             select new
                             {
